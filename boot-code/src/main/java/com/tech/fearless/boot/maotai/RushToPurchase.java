...
public class RushToPurchase implements Runnable {
    ...
    volatile static Integer cycleTimes = 0;
    ...
    public void run() {
        ...
        while (times < Start.ok) {
            ...
            try {
                if (cycleTimes < 2) {
                    gate = HttpUrlConnectionUtil.get(headers, "https://cart.jd.com/gate.action?pcount=1&ptype=1&gs=2&pid=" + Start.pid);
                    cycleTimes++;
                }
            ...
            headers.put("Referer", "https://trade.jd.com/shopping/order/getOrderInfo.action");
            ...
            try {
                if (times < Start.ok) {
                    submitOrder = HttpUrlConnectionUtil.post(headers, "https://trade.jd.com/shopping/order/submitOrder.action?&presaleStockSign=1", null);
                    Thread.sleep(50);
                } else {
